...
function readDB() {
  try {
    ...
    console.log(
      "Warning: failed on reading JSON. Returning blank list.",
      error.message,
    );
    return [];
  }
...
app.post("/sign-in", async (req, res) => {
  ...
  const logins = readDB();
  if (
    logins.find((u) => u.email === email) ||
    logins.find((u) => u.username === username)
  ) {
    return res
      .status(400)
      .json({ error: "This email/username already in use" });
  }
  try {
    const date = new Date();
    ...
    res
      .status(201)
      .json({ message: "Username created with success!", username });
  } catch (error) {
