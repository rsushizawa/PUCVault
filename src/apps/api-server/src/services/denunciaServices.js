...
module.exports = {
  ...
  async resolverDenuncia(denuncia_id, executor_id, novo_status, punicao, tempo_silencio) {
    try {
      console.log('conexão sucedida resolverDenuncia');
      await db.query(
        'CALL publico.resolver_denuncia($1, $2, $3, $4, $5)',
        [denuncia_id, executor_id, novo_status, punicao, tempo_silencio]
      );
    } catch (error) {
      errorMsg(error);
    }
