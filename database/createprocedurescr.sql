...
-- inclui tag em fórum apenas pelo criador do fórum ativo
...
-- remove tag de fórum apenas pelo criador do fórum ativo
...
-- procedure para atualizar identidade visual do fórum
-- uso: CALL publico.atualizar_identidade_visual_forum(<id do fórum>, <id do executor>, <campo ('perfil' | 'banner')>, <novo id da imagem>);
...
-- alterna o estado de autenticação em duas etapas do usuário
-- uso: CALL publico.alternar_a2f(<id do usuário>);
...
-- procedure para remover silêncio de um usuário antes do prazo
-- uso: CALL publico.remover_silencio(<id do executor>, <id do usuário>);
